...
<div class="rightSticky" data-screen="customer" >
	<ul>
		...
		<li data-type="askARepSticky" id="askARepSticky"  style="<?= ($view_settings)?( $view_settings[0]->stickyIcon_color!='')? 'background-color:'.$view_settings[0]->stickyIcon_color:'':''?>"><img src="<?=$this->project_root_url?>/cms_uploads/projects/<?=$this->project->id?>/theme_assets/sessions/conversation_icon.png" style="height:25px; width:25px"> <span><?=(isset($session->toolbox_askrep_text) && !empty($session->toolbox_askrep_text))? $session->toolbox_askrep_text: 'Ask a Rep'?></span></li>
	</ul>
</div>
