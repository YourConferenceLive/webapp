...
<script>
	$(function () {
		...
		$('#sessionsTable').on('click', '.removeSession', function () {
			...
			translationData.then((arrData) => {
				...
				// Toast
				let removeText = "has been removed!";
                ...
                Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.get(project_admin_url+"/sessions/remove/"+session_id, function (response) {
							...
							{
								listSessions();
								toastr.success(session_name+" "+removeText);
							}else{
								Swal.fire(
										errorText,
		...
		$('#pollsTable').on('click', '.redo-poll-btn', function(){
            ...
            translationData.then((arrData) => {
                ...
                // Find the translations for the dialog text
                let dialogTitle = 'Are you sure?';
                let dialogText = 'You are about to redo POLL:';
                let confirmButtonText = 'Yes, Redo it!';
                let cancelButtonText = 'Cancel';
                ...
                Swal.fire({
                    title: dialogTitle,
					html: '<span class="text-white">'+dialogText+' '+poll_id+' <br></span>',
                    icon: 'warning',
                    allowOutsideClick: false,
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
	...
	function emitTimer(pollId, that) {
		...
		translationData.then((arrData) => {
			...
			// toast
			let socketText = "Socket is not configured correctly";

			// Swal 2
			let doneText = "Done!";
			let pollText = "Poll";
			let timerText = "timer started";
			...
			for (let i = 0; i < arrData.length; i++) {
				...
				if (arrData[i].english_text === socketText) {
					socketText = arrData[i][selectedLanguage + '_text'];
				}

				if (arrData[i].english_text === doneText) {
					doneText = arrData[i][selectedLanguage + '_text'];
				}
				if (arrData[i].english_text === pollText) {
					pollText = arrData[i][selectedLanguage + '_text'];
				}
				if (arrData[i].english_text === timerText) {
					timerText = arrData[i][selectedLanguage + '_text'];
				}
			...
			$.get(project_admin_url + "/sessions/getPollByIdJson/" + pollId, function (poll) {
				...
				if (socket) {
					...
					toastr.error(socketText)
				}
				Swal.fire(
					doneText,
					pollText+' [' + pollId + '] '+timerText,
					'success'
				)
			...
			if(polls){
				if(polls.status== 'success'){
					...
					translationData.then((arrData) => {
						...
						let dialogText = 'Poll Redo Success';
